...
func day8part1() {
	...
	fmt.Println("1, 4, 7 and 8 appear", counter, "times")
}
...
func day8part2() {
	...
	for _, line := range lines {
		...
		for _, ov := range strings.Split(outputs, " ") {
			...
			outputValue = fmt.Sprintf("%s%s", outputValue, number)
		}

		val, err := strconv.Atoi(outputValue)
		if err != nil {
			fmt.Println("Could not convert output value", outputValue, err)
		} else {
			sum += val
		}
	}

	fmt.Println("The sum of all output values is:", sum)
}

func containsRunes(needle string, haystack string) bool {
...
func removeRunes(s string, target string) string {
...
var day8input = `fgcae ebafc cabdef eg abecfg abgfed feg gafdc bceg ebgcadf | defagbc faecg cfdag gecb
